...
#  Description: difficult recursions to solidify recursions
...
def groupNoAdj(start, nums, target):
    ...
    if start >= len(nums):
        return False
    if groupNoAdj(start + 2, nums, target - num[start]):
        return True
    else:
...
def splitArrayHelper(nums, a, b):

    if a == len(nums):
        return (b == 0)
    if splitArrayHelper(nums, a + 1, b + nums[a]):
        return True
    else:
        return splitArrayHelper(nums, a + 1, b - nums[a])
...
def splitOdd10Helper(nums, a, list1, list2):

    if a == len(nums):
        return list1 % 2 == 1 and list2 % 10 == 0 or list2 % 2 == 1 and list1 % 10 == 0
    if splitOdd10Helper(nums, a + 1, list1 + num[a], list2):
        return True
    else:
        return splitOdd10Helper(nums, a + 1, list1, list2 + num[a])
...
def split53Helper(nums, a, b):

    if a == len(nums):
        return b == 0
    if nums[a] % 5 == 0:
        return split53Helper(nums, a + 1, b + nums[a])
    if nums[a] % 3 == 0:
        return split53Helper(nums, a + 1, b - nums[a])
    if split53Helper(nums, i + 1, b + nums[a]):
        return True
    else:
        return split53Helper(nums, a + 1, b - nums[a])
